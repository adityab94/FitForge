...
function NutritionCard() {
  ...
  const macros = [
    { name: 'Carbs', value: carbsPct, color: '#C7522A' },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      confetti({ particleCount: 50, spread: 40, origin: { y: 0.7 }, colors: ['#C7522A', '#075056'] });
      setOpen(false);
  ...
  const handleCopyYesterday = async () => {
    ...
    try {
      ...
      confetti({ particleCount: 40, spread: 30, origin: { y: 0.7 }, colors: ['#C7522A', '#075056'] });
      setOpen(false);
    } catch (e) {
    ...
    <div className="glass-card p-4 md:p-5 anim-scale-in delay-400" data-testid="nutrition-card">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-[#C7522A]/10">
            <Utensils size={14} className="text-[#C7522A]" strokeWidth={2} />
          </div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <button
              className="btn-glow p-1.5 rounded-lg bg-[#C7522A]/10 hover:bg-[#C7522A]/20 transition-colors"
              data-testid="log-nutrition-btn"
            >
              <Plus size={14} className="text-[#C7522A]" />
            </button>
          </DialogTrigger>
          <DialogContent className="glass-card-static border-white/10 max-w-sm" style={{ background: 'linear-gradient(160deg, #FFF8F0 0%, #FFE0C0 100%)' }} data-testid="nutrition-modal">
            ...
            <button
              ...
              style={{ background: 'rgba(199,82,42,0.1)', color: 'rgba(228,238,240,0.6)', border: '1px solid rgba(199,82,42,0.15)' }}
            ...
            <div className="flex items-center gap-2 my-1">
              <div className="flex-1 h-px" style={{ background: 'rgba(199,82,42,0.12)' }} />
            ...
            <div className="flex gap-2">
              {['total', 'macros'].map(m => (
                <button key={m} onClick={() => setMode(m)} data-testid={`nutrition-mode-${m}`}
                  ...
                  style={{ background: mode === m ? '#C7522A' : 'rgba(199,82,42,0.1)', color: mode === m ? '#fff' : 'rgba(61,31,10,0.6)' }}>
              ...
              {mode === 'total' ? (
                ...
                <div className="space-y-2">
                  ...
                  {(form.carbs || form.protein || form.fat) && (
                    <p className="text-xs font-semibold" style={{ color: '#C7522A' }}>
      ...
      {showSteps ? (
        ...
        <>
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full circular-progress">
              ...
              <defs>
                <linearGradient id="activityGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#C7522A" />
          ...
          <div className="flex justify-between mt-4 text-[10px] px-2" style={{ color: 'rgba(61,31,10,0.45)' }}>
            <span>0</span>
            <span style={{ color: '#C7522A' }}>{Math.round(percentage)}%</span>
            <span>{goal} cal</span>
          </div>
...
function WorkoutsCard() {
  ...
  const handleSubmit = async (e) => {
    ...
    confetti({ particleCount: 80, spread: 60, origin: { y: 0.7 }, colors: ['#C7522A', '#C7522A', '#C7522A'] });
  ...
  return (
    <div className="glass-card p-4 md:p-5 anim-scale-in delay-600" data-testid="workouts-card">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-[#C7522A]/10">
            <Dumbbell size={14} className="text-[#C7522A]" strokeWidth={2} />
          </div>
          <span className="text-sm font-semibold" style={{ fontFamily: 'Outfit, sans-serif' }}>Workouts</span>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <button className="btn-glow p-1.5 rounded-lg bg-[#C7522A]/10 hover:bg-[#C7522A]/20 transition-colors" data-testid="add-workout-button">
              <Plus size={14} className="text-[#C7522A]" />
            </button>
          </DialogTrigger>
          <DialogContent className="glass-card-static border-white/10 max-w-md" style={{ background: 'linear-gradient(160deg, #FFF8F0 0%, #FFE0C0 100%)' }} data-testid="add-workout-modal">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-2">
              ...
              <button type="submit" className="btn-glow w-full py-3 rounded-xl text-sm font-semibold text-white transition-all" style={{ background: '#C7522A' }} data-testid="submit-workout-button">
                Add Workout
              </button>
            </form>
          </DialogContent>
              ...
              <div className="h-[50px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chestData}>
                    <Line type="monotone" dataKey="val" stroke="#C7522A" strokeWidth={2} dot={false} />
                    <YAxis hide domain={['auto', 'auto']} />
                    <XAxis hide dataKey="i" />
                  </LineChart>
                </ResponsiveContainer>
