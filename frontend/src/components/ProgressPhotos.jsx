          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {hasComparison && (
                <div className="mb-4" data-testid="before-after-comparison">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative rounded-xl overflow-hidden" style={{ border: '1px solid rgba(184,84,60,0.12)' }}>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-2" style={{ background: 'linear-gradient(transparent, rgba(255,248,240,0.9))' }}>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: 'rgba(184,84,60,0.15)', color: '#3D1F0A' }}>
                          Before · {firstPhoto.date}
                        </span>
                      </div>
                    </div>
                    <div className="relative rounded-xl overflow-hidden" style={{ border: '1px solid rgba(184,84,60,0.15)' }}>
                      <img src={getPhotoUrl(comparePhoto)} alt="After" className="w-full h-[200px] md:h-[280px] object-cover" data-testid="after-photo" />
                      <div className="absolute bottom-0 left-0 right-0 p-2" style={{ background: 'linear-gradient(transparent, rgba(255,248,240,0.9))' }}>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: 'rgba(184,84,60,0.15)', color: '#B8543C' }}>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                {photos.map((photo) => (
                  <div key={photo.id} className="relative group rounded-xl overflow-hidden aspect-square" style={{ border: '1px solid rgba(184,84,60,0.1)' }} data-testid={`progress-photo-${photo.id}`}>
